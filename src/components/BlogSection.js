...
const blogData = [
  {
    ...
    excerpt: 'Aaditya Chowdhury shares his journey from an intern to a successful frontend developer and entrepreneur while in college.',
  ...
  },
  {
    ...
    excerpt: 'A simple explanation of the event loop in JavaScript, making it easy to understand for beginners.',
    date: 'October 16, 2024',
  ...
  },
  {
    id: 3,
    title: 'Coming Soon',
    excerpt: 'Stay tuned for my upcoming article on optimizing large language models for production environments.',
    date: 'TBA',
...
const BlogSection = () => {
  return (
    <motion.div
      ...
      className="blog_section tw-flex tw-flex-col tw-items-start tw-bg-gray-900 tw-rounded-xl tw-shadow-lg tw-p-8 tw-w-full md:tw-w-4/5 lg:tw-w-full tw-mt-10 tw-mb-10"
    >
      ...
      >
        Thoughts, experiences, and technical insights from my journey in software engineering
      </motion.p>
      ...
      <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 lg:tw-grid-cols-3 tw-gap-6 tw-mt-4 tw-w-full">
        {blogData.map((blog, index) => (
          ...
          >
            ...
            >
              <div className="tw-bg-black/30 tw-overflow-hidden tw-rounded-xl tw-border tw-border-neutral-800 tw-h-full tw-flex tw-flex-col tw-transition-all tw-duration-300 hover:tw-shadow-[0_0_15px_rgba(79,189,185,0.3)] hover:tw-border-[#4fbdb9]/50 tw-group-hover:tw-transform tw-group-hover:tw-scale-[1.02]">
                ...
                <div className="tw-flex tw-p-5 tw-flex-col tw-flex-grow">
                  {/* Date */}
                  <span className="tw-text-sm tw-text-[#4fbdb9] tw-mb-2">{blog.date}</span>
                  ...
                  <div className="tw-flex tw-items-center tw-text-sm tw-text-[#4fbdb9] tw-font-medium">
                    {blog.external ? 'Read on ' + new URL(blog.link).hostname.replace('www.', '') : 'Read more'}
                    <svg 
